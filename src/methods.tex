...
The reference image was the standard MNI152 atlas, padded by 50\% in the inferior direction because the original atlas excludes the subcranial region where ICAs are.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		\node[mybox, below=of thresh] (growing) {III) Largest 2 Volumes};
...
\subsubsection{Noise}
Accurate noise modelling is challenging because multiple sources contribute to the variance.
...
While raw PET counts are Poisson-distributed, post-reconstruction noise is typically approximated as Gaussian \cite{teymurazyan2013properties}.
...
\end{equation}
Thus, \(\sigma^2\) can be sampled directly without Metropolis–Hastings \cite{irace2020bayesian}.
...
For \yohimbine, kinetic modelling requires the plasma parent activity, so whole-blood IDIFs were converted to plasma and corrected for metabolites using
\begin{equation}
...
For each subject, first, the Hammersmith brain atlas subregions were merged into larger regions: white matter (WM), gray matter (GM), gray nuclei (GN), cerebellar gray matter (CGM), and cerebellar white matter (CWM).
An algorithm originally developed for pseudo-CT generation from T1-weighted MRI for bone attenuation correction in PET was used to create masks for the extracerebral regions\cite{mérid2017}.
Values above 300 HU were labeled as bone (BONE) and values between -300 HU and 300 HU after excluding the brain as extracerebral soft tissues (SOFT), which include the scalp, neck muscles, glands, and eyes.
...
Our SOFT definition is broad; while adequate for simulation, it violates homogeneity assumptions in compartment modelling, so the unfitted TAC was retained for SOFT.
...
\begin{figure}[h]
	...
	\begin{tikzpicture}[
		...
		\node[right=0cm of phantom_image] (ttac_image) {\includegraphics[height=3cm]{figures/ttacs.png}};
