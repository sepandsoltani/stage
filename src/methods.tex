...
\]
Where $\circledast$ denotes the convolution operator, \(\alpha_i\) is the coefficient. and \(\beta_i\) is the frequency of th \(i\)th spectrum.

The number of basis functions (\(s\)) should be selected so it can accurately describe the activity of the tracer while avoiding being chosen too high to avoid over complicating and also introducing noise.
...
We can derive the observed data from the GTM response of the model \ref{TODO}. For convinience, we define
\[
	\mathcal{T}(t) = \mathcal{G}(t;\Theta) =
	\mathcal{G}\left(
	\begin{bmatrix}
			T_{IF}(t;\theta_{1}, \theta_{2}, \theta_{3}) \\
			T_{tissue}(t;\alpha_{1}, \beta_{1}, \dots)
		\end{bmatrix}\right)
\]
where $\mathcal{G}$ is the GTM model and $\mathcal{T}$ is the percieved activity based on the GTM model.
\[
    L(\Theta, \sigma^2) = \prod_{i=1}^N \frac{1}{\sqrt{2\pi \sigma^2}} \exp\left( -\frac{(\mathcal{D}(t_i) - \mathcal{T}(t_i))^2}{2\sigma^2} \right)
\]
