...
Some sources of noise such as scattering effect, random events, and motion artifacts among others are considered corrected for by the reconstruction program.
However the signal in the artery suffers from a special case since they are heavily effected by "low-count statistics" caused by short time frames at beginning of the scan when the changes in the input function are very rapid and by exponential decay of the radiotracer at the end of the scan.

Noise in raw PET counts is considered to be a Poisson distribution however after the reconstruction process it is widely assumed to be a Gaussian distribution \cite{TODO}. To account for the previously mentioned time-variant noise, weights are used to normalize the noise along all frames to a single a statistic. We define the weights as
\[
	\omega_{i} = \frac{\Delta t_i}{C_T(t_i)} e^{\frac{-t_{i} ln(2)}{T_{1/2}}}
\]
where \(\omega_i\) is the weight and \(\Delta t_i\) is the frame duration of the \(i\)th frame, \(C_T(t_i)\) is the total radioactivity concentration at the mid-time frame, and \(T_{1/2}\) is the half-life of the radioisotope. \(C_T(t_i)\) can also be substituted with \(c_i\), the net-true count of the PET frame.

The time-variant noise can be normalized as the weighted mean of the per frame noise:

\[
    \epsilon = \frac{1}{N} \sum_{i=1}^{N} \omega_i \epsilon_{i}.
\]

\subsubsection{Estimation}
Let us define all model parameters in to the vector \( \Theta = (\theta_{1}, \theta_{2}, \theta_{3}, \alpha_{1}, \beta_{1}, \dots) \) and \(\mathcal{D}\) as the observed PET TAC.
